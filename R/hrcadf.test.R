...
hrcadf.test <- function(x=NULL,
    ...
    if(adf.type=="nc") {
        ...
        adf.type <- c("none", "drift", "trend")        
    }
    df.type <- match.arg(df.type)
    if(df.type=="nc") {
        df.type <- c("none") 
    } else if(df.type=="ncc") {
        df.type <- c("none", "drift")
    } else if(df.type=="nccct") {
        df.type <- c("none", "drift", "trend")        
    ...
    for(t in df.type) {
        out <- suppressWarnings(CADFtest(x,max.lag.y=0,type=t))
        r <- Dmat.func(out$est.model,method=method) 
        if(!exists("ma.mat")) {
            ma.mat <- as.matrix(r)
            rank.vec <- out$est.model$rank
            t.stat <- out$statistic
        } else {
            ma.mat <- cbind(ma.mat,r)            
            rank.vec <- c(rank.vec,out$est.model$rank)
            t.stat <- c(t.stat,out$statistic)
        }
    }
    ...
    for(k in 1:K) {
        for(t in adf.type) {
            out <- suppressWarnings(CADFtest(x,max.lag.y=lag.vec[k],type=t))
            r <- Dmat.func(out$est.model,method=method) 
            if(!exists("ma.mat")) {
                ma.mat <- as.matrix(r)
                rank.vec <- out$est.model$rank
    ...
    M.dim <- ncol(ma.mat)
    sigsq.largest <- summary(out$est.model)$sigma**2
    ...
    for(b in 1:B) {
        ...
        t.stat.boot <- NULL
        for(t in df.type) {
            t.stat.boot <- c(t.stat.boot,suppressWarnings(CADFtest(x.boot,max.lag.y=0,type=t)$statistic))
        }        
        
        for(k in 1:K) {
            for(t in adf.type) {
                t.stat.boot <- c(t.stat.boot,suppressWarnings(CADFtest(x.boot,max.lag.y=lag.vec[k],type=t)$statistic))
            }
        }
