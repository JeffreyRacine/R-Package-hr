...
jackknife model averaging procedure who propose a jackknife criterion
for the model average estimator given by
\begin{equation*}
  CV_{n}(w)=\frac{1}{n}w'\tilde E'\tilde Ew
\end{equation*}
where $\tilde E$ is the $T\times M$ matrix with columns containing the
jackknife residual vector from the $m$th candidate estimating equation
formed by deleting the $t$ observation when constructing the $t$th
residual. Both the Mallows and jackknife criterion could be used in
what follows.
