%% $Id: bmmaur.tex,v 1.14 2017/03/17 11:26:37 jracine Exp jracine $
...
average of those taken from a set of candidate estimating equations
that are the same as those used for \possessivecite{NG_PERRON:2001} or
\possessivecite{DICKEY_FULLER:1979} approaches. We also adopt an
automatic, model-free, time series bootstrap in order to construct the
null distribution of this statistic and compute nonparametric critical
values or nonparametric $P$-values.  Most existing bootstrap unit root
procedures are model-based, and one problem with model-based
resampling is that the data generating process (DGP) is unknown and
must be identified from the series at hand. In order to ensure that
the bootstrap samples have the same structure as the series at hand,
this identification must be correct. \citeasnoun{SWENSEN:2003}
considers model-based and model-free bootstrap approaches and
demonstrates that a difference-based model-free approach (along the
lines of that proposed herein) delivers a bootstrap distribution that
approaches the true asymptotic distribution under the null of a unit
root (see also \citeasnoun[Section 2.4]{PALM_ET_AL:2008}).
...
see \citeasnoun{Hansen2010142} for the use of a Mallows criterion
\cite{MALLOWS:1973} for combining forecasts local to a unit root)). To
the best of our knowledge, ours is the first to consider model
averaging procedures in the unit root setting. For the MMA-based unit
...
\subsection{The Mallows Criterion for the Model Average Estimator}

The goal in model averaging is to reduce estimation variance while
controlling misspecification bias. The Mallows Criterion for the model
average estimator \cite{HANSEN:2007} is
\begin{equation*}
...
further details and see also \citeasnoun{HANSEN_RACINE:2012} for a
jackknife model averaging procedure.
...
\begin{table}[!ht]
  \caption{\label{tab:norm_size}Normed size distortions
    ...
    tables)}
\input /Users/jracine/tex/categorical/bootstrap_unitroot/work/tex/norm_size_no_trend
\quad
\input /Users/jracine/tex/categorical/bootstrap_unitroot/work/tex/norm_size_trend
\end{table}
...
\begin{table}[!ht]
  \caption{\label{tab:norm_power}Normed power $||\hat\alpha-1||$ for the
    ...
    contain deterministic trends, the P-P test contains a trend in both
    tables)}
\input /Users/jracine/tex/categorical/bootstrap_unitroot/work/tex/norm_power_no_trend
\quad
\input /Users/jracine/tex/categorical/bootstrap_unitroot/work/tex/norm_power_trend
\end{table}
