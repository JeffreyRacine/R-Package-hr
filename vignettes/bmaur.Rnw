%% $Id: bmmaur.tex,v 1.16 2017/03/18 20:37:09 jracine Exp jracine $
...
We compare the proposed bootstrap MA approach with the classic
...
proposed bootstrap model average (MA)
...
from each of the $M$ candidate estimating equations and average them
using the weight vector $\hat w$, and call this averaged statistic
$\mathcal{T}_M=\sum_{m=1}^M\hat w_m\tau_m$. In order to obtain the
...
the sampling distribution of $\mathcal{T}_M$ under the null of a unit root
...
\item Next, take the bootstrap $\tau^*$ statistics from each of the
  $M$ candidate estimating equations and average them using the weight
  vector for the original series $\hat w$, which delivers a bootstrap
  model average statistic $\mathcal{T}^*_M$ generated under the null

\item Repeat this process $B$ times, then compute percentiles of the
  $B$ bootstrap statistics
  $\mathcal{T}^*_{1,M},\dots, \mathcal{T}^*_{B,M}$ which serve as
  critical values under the null (e.g., $\alpha/2$ and $1-\alpha/2$
  where $\alpha$ is the desired level of the test); finally, determine
  whether one will reject or fail to reject at level $\alpha$
