...
\caption{\label{tab:maic_mma_comp}Empirical rejection probabilities
  ...
  be found in Appendix \ref{app:R}.}
\input /Users/jracine/tex/categorical/bootstrap_unitroot/work/Schwert_12_trend/tex/reject_3f
\quad
...
\begin{table}[!ht]
  \caption{Empirical rejection probabilities
    ...
    Model Averaging test (MMA). The N-P, MMA, and ADF-BIC methods use
    the same set of candidate models determined by Schwert's ad-hoc lag
    selection rule, $k=1,\dots,j(N/100)^{1/4}$, $j=4,8,12,24$
...
\begin{table}[!ht]
  \caption{Empirical rejection probabilities
    ...
    the same set of candidate models determined by Schwert's ad-hoc lag
    selection rule, $k=1,\dots,j(N/100)^{1/4}$, $j=4,8,12,24$
    (clockwise from top left), no deterministic trend in any model.}
\Tiny
